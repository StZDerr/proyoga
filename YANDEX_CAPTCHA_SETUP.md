...
## ✅ Капча уже настроена и работает!

### Что защищено:

-   📝 Форма записи на занятие (главная страница)
-   🧪 Модальная форма теста
-   📧 Контактная форма

### Готовые компоненты:

-   `<x-yandex-captcha />` - Blade компонент
-   Валидация на сервере через `YandexCaptcha` правило
-   JavaScript интеграция в формах

## 🔧 Настройка для нового домена

### 1. Обновите домен в Яндекс.Облако

1. Перейдите в [консоль Яндекс.Облако](https://console.cloud.yandex.ru/)
2. Выберите сервис **SmartCaptcha** → ваша капча
3. Добавьте новый домен сайта в **"Список разрешённых доменов"**

### 2. Проверьте ключи в .env

```env
# Яндекс SmartCaptcha
YANDEX_CAPTCHA_CLIENT_KEY=ysc1_...ваш_client_key
YANDEX_CAPTCHA_SERVER_KEY=ysc2_...ваш_server_key
```
...
# Тестовое уведомление (все каналы)
php artisan test:notifications "Тест капчи на продакшене"
```

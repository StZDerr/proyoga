...
@section('content')
    <div class="container">
        ...
        <form action="{{ isset($priceItem) ? route('price-items.update', $priceItem) : route('price-items.store') }}"
            method="POST">
            @csrf
            @if (isset($priceItem))
                @method('PUT')
                ...
                <select name="table_id" class="form-select" required>
                    ...
                    @foreach ($tables as $table)
                        <option value="{{ $table->id }}" @if (isset($priceItem) && $priceItem->table_id == $table->id) selected @endif>
                            {{ $table->title }} ({{ $table->category->name }})
                        </option>
                    @endforeach
                </select>
            </div>

            <button type="submit" class="btn btn-success">{{ isset($priceItem) ? 'Сохранить' : '

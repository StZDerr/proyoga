...
@section('content')
    <div class="container">
        ...
        <form
            action="{{ isset($priceTable) ? route('admin.price-tables.update', $priceTable) : route('admin.price-tables.store') }}"
            method="POST">
            @csrf
            @if (isset($priceTable))
                @method('PUT')
            ...
            <a href="{{ route('admin.price-tables.index') }}" class="btn btn-secondary">Назад</a>
        </form>
    </div>
@endsection

        ...
        <div class="container-xl">
            <div class="row row-deck row-cards">
                <div class="col-lg-8">
                    ...
                    <!-- Предварительный просмотр в sitemap -->
                    @if ($indexing->is_indexed)
                        <div class="card mt-3">
                            <div class="card-header">
                                <h3 class="card-title">Предварительный просмотр в sitemap.xml</h3>
                            </div>
                            <div class="card-body">
                                <div class="highlight">
                                    <pre><code class="language-xml">&lt;url&gt;
    &lt;loc&gt;{{ url($indexing->url) }}&lt;/loc&gt;
    &lt;lastmod&gt;{{ $indexing->last_modified ? $indexing->last_modified->format('Y-m-d') : now()->format('Y-m-d') }}&lt;/lastmod&gt;
    &lt;changefreq&gt;{{ $indexing->changefreq }}&lt;/changefreq&gt;
    &lt;priority&gt;{{ $indexing->priority }}&lt;/priority&gt;
&lt;/url&gt;</code></pre>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="card mt-3">
                            <div class="card-body">
                                <div class="alert alert-warning mb-0">
                                    <h4 class="alert-title">Страница не включена в sitemap</h4>
                                    <div class="text-muted">
                                        Эта страница не будет добавлена в sitemap.xml, поскольку индексация отключена.
                                    </div>
                                </div>
                            </div>
                        </div>
                                    ...
                                    <div class="row align-items-center">
                                        <div class="col-auto">
                                            <span class="bg-warning text-white avatar">
                                                <i class="fas fa-toggle-{{ $indexing->is_indexed ? 'on' : 'off' }}"></i>
                                            </span>
                                        </div>
                                        <div class="col text-truncate">
                                            <strong>{{ $indexing->is_indexed ? 'Отключить' : 'Включить' }}
                                                индексацию</strong>
                        ...
                        <div class="card-body">
                            ...
                            @if ($indexing->last_modified)
                                <div class="mb-3">
                                    ...
                                    <div class="font-weight-medium">{{ $indexing->last_modified->format('d.m.Y H:i') }}
                                    </div>
                                </div>
                            @endif
                        </div>
